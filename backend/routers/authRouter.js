...
router.post("/:handle", async (req, res) => {
    try {
        ...
        // sign the token
        const token = jwt.sign({ user: savedUser._id, type: handle }, 'abcde');
...
router.post("/login/:handle", async (req, res) => {
    ...
    try {
        ...
        if (!passwordCorrect)
            return res.status(401).json({ errorMessage: "Wrong username or password." });
        console.log("NO error  at this line")
...
router.get("/loggedIn", async (req, res) => {
    try {
        ...
        const user = await (verified.type == "bank" ? bloodbanks : User).findOne({ _id: verified.user }, { password: 0, donations: 0, requests: 0, stock: 0, __v: 0 });
